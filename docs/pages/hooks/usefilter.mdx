...
| `params` | `{sort: 'alpha'` \| `'count'` \| `'none, sortAscending: boolean}` | `{sort: 'alpha', sortAscending:` `true`}` | `'alpha'` stands for `'alphanumeric'` meaning to sort the items based on label alphabetically, `'count'` to sort based on count, `'none'` to skip the sorting process. |
