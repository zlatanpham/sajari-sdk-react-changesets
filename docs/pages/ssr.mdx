...
If you're using `@tpham0/search-ui`...

```jsx live=false
import { SSRProvider, SearchProvider } from '@tpham0/search-ui';
...
If you're using `@tpham0/search-components` with no search logic required...

```jsx live=false
import { SSRProvider } from '@tpham0/search-components';
...
In order to render the components or use hooks while server side rendering, you need a way to fetch data. For this, you can use the `server` function from the `@tpham0/server` package. It expects the same props as `SearchProvider`.
